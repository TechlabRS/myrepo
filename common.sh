...
func_stat_check() {
  if [ $1 -eq 0 ]; then
    ...
    echo "Refer the log file $log_file for more information"
    exit 1
  fi
}
...
func_schema_setup() {
  if [ "$schema_setup" == "mongo" ]; then
    ...
    yum install mongodb-org-shell -y &>>$$mylog
    func_stat_check $?

    func_print_head "Load Schema"
    mongo --host dev-mongodb.pearlwebsolutions.in </app/schema/${component}.js &>>$log_file
    func_stat_check $?
  fi
  if [ "${schema_setup}" == "mysql" ]; then
    ...
    func_print_head "Load Schema"
    mysql -h dev-mysql.pearlwebsolutions.in -uroot -p${mysql_root_password} < /app/schema/shipping.sql &>>$log_file
    func_stat_check $?
  fi
}
